...
document.addEventListener('DOMContentLoaded', function() {
    ...
    checkWebcamStatus();
    setInterval(checkWebcamStatus, 5000);
    ...
    startButton.addEventListener('click', function() {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                addLogEntry('Webcam iniciada');
            } else {
                console.error('Error al iniciar la webcam:', data.message);
                addLogEntry('Error al iniciar la webcam: ' + data.message);
            }
        })
        .catch(error => {
            console.error('Error:', error);
            addLogEntry('Error al comunicarse con el servidor');
        });
    });
    ...
    stopButton.addEventListener('click', function() {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                // Agregar evento al registro
                addLogEntry('Webcam detenida');
            } else {
                console.error('Error al detener la webcam:', data.message);
                addLogEntry('Error al detener la webcam: ' + data.message);
            }
        })
        .catch(error => {
            console.error('Error:', error);
    ...
    captureButton.addEventListener('click', function() {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                // Agregar evento al registro
                addLogEntry('Imagen capturada');
            } else {
                console.error('Error al capturar imagen:', data.message);
                addLogEntry('Error al capturar imagen: ' + data.message);
            }
        })
        .catch(error => {
            console.error('Error:', error);
